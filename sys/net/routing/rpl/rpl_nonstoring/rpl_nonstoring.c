...
void rpl_send_DIO_mode(ipv6_addr_t *destination)
{
    ...
    rpl_send_dio_buf->rank = HTONS(mydodag->my_rank);
    ...
    rpl_send_opt_dodag_conf_buf->MaxRankIncrease = HTONS(mydodag->maxrankincrease);
    rpl_send_opt_dodag_conf_buf->MinHopRankIncrease = HTONS(mydodag->minhoprankincrease);
    rpl_send_opt_dodag_conf_buf->ocp = HTONS(mydodag->of->ocp);
    ...
    rpl_send_opt_dodag_conf_buf->lifetime_unit = HTONS(mydodag->lifetime_unit);
...
void rpl_recv_DIO_mode(void)
{
    ...
    DEBUGF("instance %04X rank %04X\n", rpl_dio_buf->rpl_instanceid, NTOHS(rpl_dio_buf->rank));
    ...
    while (len < (NTOHS(ipv6_buf->length) - ICMPV6_HDR_LEN)) {
        ...
        switch (rpl_opt_buf->type) {
            ...
            case (RPL_OPT_DODAG_CONF): {
                ...
                dio_dodag.maxrankincrease = NTOHS(rpl_opt_dodag_conf_buf->MaxRankIncrease);
                dio_dodag.minhoprankincrease = NTOHS(rpl_opt_dodag_conf_buf->MinHopRankIncrease);
                dio_dodag.default_lifetime = rpl_opt_dodag_conf_buf->default_lifetime;
                dio_dodag.lifetime_unit = NTOHS(rpl_opt_dodag_conf_buf->lifetime_unit);
                dio_dodag.of = (struct rpl_of_t *) rpl_get_of_for_ocp(NTOHS(rpl_opt_dodag_conf_buf->ocp));
    ...
    if (my_dodag == NULL) {
        ...
        if (NTOHS(rpl_dio_buf->rank) < ROOT_RANK) {
        ...
        if (NTOHS(rpl_dio_buf->rank) != INFINITE_RANK) {
            DEBUGF("Will join DODAG\n");
            rpl_join_dodag(&dio_dodag, &ipv6_buf->srcaddr, NTOHS(rpl_dio_buf->rank));
            DEBUGF("Joined DODAG\n");
            DEBUGF("The DODAG-root is: %s\n", ipv6_addr_to_str(addr_str_mode, IPV6_MAX_ADDR_STR_LEN,
                    &dio_dodag.dodag_id));
        }
    ...
    if (rpl_equal_id(&my_dodag->dodag_id, &dio_dodag.dodag_id)) {
        ...
        if (RPL_COUNTER_GREATER_THAN(dio_dodag.version, my_dodag->version)) {
            if (NTOHS(my_dodag->my_rank) == ROOT_RANK) {
                ...
                rpl_global_repair(&dio_dodag, &ipv6_buf->srcaddr, NTOHS(rpl_dio_buf->rank));
            }
    ...
    if (parent == NULL) {
        ...
        parent = rpl_new_parent(my_dodag, &ipv6_buf->srcaddr, NTOHS(rpl_dio_buf->rank));
    ...
    /* update parent rank */
    parent->rank = NTOHS(rpl_dio_buf->rank);
